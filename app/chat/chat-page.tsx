  ...
  if (messagesNode.length <= 0) {
    return (
      <div className="flex h-screen w-full flex-col items-center justify-center bg-white dark:bg-slate-950">
        <div className="h-8 w-8 animate-spin rounded-full border-b-2 border-blue-500 dark:border-indigo-400"></div>
        <p className="mt-2 text-gray-500 dark:text-slate-400">加载中...</p>
      </div>
    );
  }

  return (
    <div className="flex relative h-screen w-full flex-col items-center justify-center overflow-hidden p-6 bg-white dark:bg-slate-950">
      <div className="absolute z-50 top-6 left-0 box-border px-6 w-full h-4 bg-gradient-to-r from-white dark:from-slate-950 to-transparent">
        <div className="bg-gradient-to-b from-white dark:from-slate-950 to-transparent w-full h-full"></div>
      </div>
      <div
        ref={scrollDivRef}
...
export default function ChatPage() {
  ...
  if (!currentConversationId) {
    return (
      <div className="flex h-full w-full flex-col items-center justify-center p-6 bg-white dark:bg-slate-950">
